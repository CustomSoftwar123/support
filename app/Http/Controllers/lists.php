...
class lists extends Controller
{
    ...
    public function List(Request $request)
    {
        ...
        // if($ListType == 'Roles') { $ListType = 'User Roles'; }
           $code = DB::table('lists')->where('ListType','ROL')->pluck('Code')->toArray(); 
//  $code[0]->Code;
        // if(count($code) == 0) {
        //      return redirect('/home');
        // }
         ...
         if ($request->ajax()) {
            ...
            $data = DB::table('lists') 
                         ->select('lists.id',
                            DB::raw('(select count(ListOrder) from lists where ListType = "ROL") as MaxListOrder'),
                            'lists.Text',
                            'lists.InUse',
                            'lists.Code',
                            'lists.ListType',
    ...
    return $data = DB::table('')->where('ListType', 'List')->where('InUse', 1)->orderBy('ListOrder')->get();
